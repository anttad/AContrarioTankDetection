...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar  6 17:06:31 2020
...
from conversion_pix_lonlat import pix_to_lon_lat,lonlat_to_pix
import os
import numpy as np 
import cv2
import json 
...
from skimage.draw import circle_perimeter, rectangle_perimeter,rectangle,circle, polygon_perimeter,polygon
...
def detect_tanks(B02, B03, B04, save_res_img=False,iso_th=0.9,low=400,high=700,zoom=False,fold_path='./'):    
    """
    im_path : absolute path to a tif image with bands B02, B03, B04
...
#    devernay = os.path.join(c_path,"devernay")
    devernay = "~/Documents/THESE_CMLA/CodeV0/Devernay_Ipol/devernay_1.1/build/devernay"
...
#    par_res_folder = '/home/l.carvalho/data/tanks_filipinas/res_iso_{}_l_{}_h_{}_zoom_{}'.format(iso_th,low,high,zoom)
    fold_name = "res_iso_{}_l_{}_h_{}__zoom_{}".format(iso_th,low,high,zoom)
    ...
    try:
        b02_img = skimage.io.imread(B02)
        b03_img = skimage.io.imread(B03)
        b04_img = skimage.io.imread(B04)
...
#    img[:,:,3] = b08_img
    ...
    pdf = os.path.join(edge_folder,"test.pdf")
...
#    im_dim = output.shape
    ...
    for cluster in clusters:     
        if cluster['meaningful']:
            ...
            lat1, lon1 = pix_to_lon_lat(B02,rec1[0], rec1[1])
            lat2, lon2 = pix_to_lon_lat(B02,rec2[0], rec2[1])
    ...
    print("full processing done in {:.2f}s".format(time()-process_time))
    if save_res_img : 
        input_img = img[:,:,:-1]
...
#def detect_tanks(im_path, meta_tif=None, save_res_img=False):
def detect_tanks_planet(planet_img, save_res_img=False,iso_th=0.9,low=400,high=700,sigma=1,zoom=False,fold_path='./', auto_th=False):    
    """
    im_path : absolute path to a tif image with bands B02, B03, B04
    meta_tif : optional tif image with available meta_data (if the one in im_path are absent)
    save_res_img : save the result image if set to True
...
#    par_res_fold = "/home/antoine/Documents/Lucas_Code/tank_detection_pipeline/TankDetection/res_iso_{}_l_{}_h_{}_zoom_{}".format(iso_th,low,high,zoom)
#    par_res_folder = '/home/l.carvalho/data/tanks_filipinas/res_iso_{}_l_{}_h_{}_zoom_{}'.format(iso_th,low,high,zoom)
    fold_name = "res_planet_iso_{}_l_{}_h_{}_s_{}_zoom_{}_auto_{}".format(iso_th,low,high,sigma,zoom,auto_th)
    par_res_fold = os.path.join(fold_path,fold_name)
    os.system('mkdir {}'.format(par_res_fold))
    res_folder = os.path.join(par_res_fold,im_name)
    try: 
    ...
    try:
        full_img = skimage.io.imread(planet_img)
#        with rasterio.open(B08) as src:
#            b08_img = src.read(1, window=win)
    except:
        print('image corrupted')
        return
    w_aux, h_aux, _ = full_img.shape
#    img = np.zeros((w_aux, h_aux, 3))
#    img[:,:,0] = b02_img
#    img[:,:,1] = b03_img
#    img[:,:,2] = b04_img
#    img[:,:,3] = b08_img
    ...
    print("PCA reduction...")
    pca_time = time()
    img_pca = channel_reduction_pca(full_img)
    if zoom:
        print("fft zoom...")
        zoom_time=time()
        img_pca = fftzoom(img_pca)
    ...
    edges_time = time()
    print(os.getcwd())
    os.system("{} {}  -l {} -h {} -s {} -p {} -t {}".format(devernay, im_pca_path, low, high,sigma, pdf, txt)) 
    print(txt)
    #circles = []
    print("Edge extraction done in {:.2f}s".format(time()-edges_time))
    print('             ')
...
#    im_dim = output.shape
    ...
    if save_res_img : 
        ...
        output_pol = np.uint8(255*((output_pol - output_pol.min())/(output_pol.max() - output_pol.min())))
        pred_mask = np.zeros((output_pol.shape[0], output_pol.shape[1]))
        for seg, iso_rat in seg_iso_per_rat:
            
            x,y = seg[:,1], seg[:,0]
            rx, ry  = polygon_perimeter(x,y,output_pol.shape)
            recx, recy  = polygon(x,y,output_pol.shape)
            if iso_rat >= iso_th :
                output_pol[rx,ry] = (0,255,0)
                pred_mask[recx,recy] = 1
            else :
                output_pol[rx,ry] = (255,0,0)
#            
        skimage.io.imsave(os.path.join(output_pol_fold,im_name+'.png'), np.uint8(output_pol))
        skimage.io.imsave(os.path.join(output_pol_fold,'mask_'+im_name+'.png'), pred_mask)
...
#    folder = "/home/antoine/Documents/Lucas_Code/tank_detection_pipeline/single_test"
    iso_th = 0.9
    sigma=3
    folder = "/home/antoine/Documents/THESE_CMLA/CodeV0/Devernay_Ipol/devernay_1.1/build"
    B02 = os.path.join(folder, "2019-06-15_S2A_orbit_046_tile_51RVP_L1C_band_B02.tif")
    B03 = os.path.join(folder, "2019-06-15_S2A_orbit_046_tile_51RVP_L1C_band_B03.tif")
    B04 = os.path.join(folder, "2019-06-15_S2A_orbit_046_tile_51RVP_L1C_band_B04.tif")
#    latlon_list = detect_tanks(B02, B03, B04, iso_th=iso_th, fold_path = folder,save_res_img=True)
    
    planet_img = "/home/antoine/Documents/tsd/tsd/aoshan_planet/2019-07-27_scene_20190727_011259_0f21.tif"
    
#    latlo1n_ar1r = np.array(latlon_list)
#    
#    lat_max = latlon_arr[:,0].max()
#    lon_max = latlon_arr[:,1].max()
#    lat_min = latlon_arr[:,0].min()
#    lon_min = latlon_arr[:,1].min()
#    row1, col1 = lonlat_to_pix(planet_img, lon_max,lat_max)
#    row2, col2 = lonlat_to_pix(planet_img, lon_min,lat_min)
#    crop = [[row1,col1], [row2,col2]]
#    
#    img = skimage.io.imread(planet_img)
#    r_max, c_max,_ = img.shape
#    cr_img = img/img.max()
#    cr_img = cr_img[min((0,row1,row2)):max((r_max,row1,row2)),min((0,col1,col2)):max((c_max,col1,col2))]
#    plt.figure()
#    plt.imshow(cr_img)
#    
    detect_tanks_planet(planet_img,iso_th=iso_th,fold_path=folder,save_res_img=True,sigma=sigma, auto_th=False)
